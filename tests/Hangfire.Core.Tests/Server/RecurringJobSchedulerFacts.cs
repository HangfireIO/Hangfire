...
// ReSharper disable once AssignNullToNotNullAttribute
        ...
        [Theory]
        [InlineData(-1)]
        [InlineData(0)] // Should be inclusive
        public void Execute_EnqueuesAJob_WhenInBoundsOfStartDate(int nowOffsetSeconds)
        {
            var startDate = _nowInstant.AddSeconds(nowOffsetSeconds);
        ...
        {
            var endDate = _nowInstant.AddSeconds(nowOffsetSeconds);
        ...
        {
            var startDate = _nowInstant.AddSeconds(1);
            _recurringJob["StartDate"] = JobHelper.SerializeDateTime(startDate);
            var scheduler = CreateScheduler();
        ...
        {
            var endDate = _nowInstant.AddSeconds(-1);
            _recurringJob["EndDate"] = JobHelper.SerializeDateTime(endDate);
            var scheduler = CreateScheduler();
            ...
            _connection.Verify(x => x.SetRangeInHash(
                $"recurring-job:{RecurringJobId}",
                It.Is<Dictionary<string, string>>(rj =>
                    rj.ContainsKey("NextExecution") && rj["NextExecution"] == null)));
        }
