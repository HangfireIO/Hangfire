...
namespace Hangfire.Core.Tests.Server
{
    public class RecurringJobSchedulerFacts
    {
        ...
        public RecurringJobSchedulerFacts()
        {
            ...
            _connection.Setup(x => x.GetFirstByLowestScoreFromSet("recurring-jobs", 0, It.Is<double>(value => JobHelper.ToTimestamp(_nowInstant) <= value && value <= JobHelper.ToTimestamp(_nowInstant.AddSeconds(5)))))
            ...
            _connection.SetupSequence(x => x.GetFirstByLowestScoreFromSet("recurring-jobs", 0, It.Is<double>(value => JobHelper.ToTimestamp(_nowInstant) <= value && value <= JobHelper.ToTimestamp(_nowInstant.AddSeconds(5))), It.IsAny<int>()))
