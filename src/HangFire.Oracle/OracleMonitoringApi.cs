    ...
    public class OracleMonitoringApi : SqlMonitoringApi {
        ...
        public override StatisticsDto GetStatistics() {
            return UseConnection((connection,transaction) => {
                ...
                stats.Deleted = Convert.ToInt64(connection.Query<int?>(sqls[3], transaction: transaction).SingleOrDefault());
        ...
        protected override Dictionary<string, long> ExecuteHourlyTimelineStatsQuery(IDbConnection connection, IDbTransaction transaction, List<string> keys) {
            return GetStats(connection, transaction, SqlBook.SqlMonitoringApi_GetHourlyTimelineStats, keys);
        }

        protected override Dictionary<string, long> ExecuteGetTimelineStatsQuery(IDbConnection connection, IDbTransaction transaction, List<string> keys) {
            return GetStats(connection, transaction, SqlBook.SqlMonitoringApi_GetTimelineStats, keys);
        }

        private Dictionary<string, long> GetStats(IDbConnection connection, IDbTransaction transaction, string sql, List<string> keys) {
