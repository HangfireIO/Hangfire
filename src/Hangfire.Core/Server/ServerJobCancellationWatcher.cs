...
namespace Hangfire.Server
{
    internal sealed class ServerJobCancellationWatcher : IBackgroundProcess
    {
        ...
        {
            context.Logger.Trace("Checking for aborted jobs...");
            ...
            using (var connection = context.Storage.GetConnection())
            {
                ...
                {
                    context.Logger.Debug($"Job {abortedJobId.Item1} was aborted on worker {abortedJobId.Item2}.");
                    aborted = true;
                }

                if (!aborted)
                {
                    context.Logger.Trace("No newly aborted jobs found.");
                }
            }
