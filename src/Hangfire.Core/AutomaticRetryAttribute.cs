...
namespace Hangfire
{
    ...
    public sealed class AutomaticRetryAttribute : JobFilterAttribute, IElectStateFilter, IApplyStateFilter
    {
        ...
        private void ScheduleAgainLater(ElectStateContext context, int retryAttempt, FailedState failedState)
        {
            ...
            if (LogEvents)
            {
                Logger.WarnException(
                    "Failed to process the job '{backgroundJobId}': an exception occurred. Retry attempt {retryAttempt} of {attempts} will be performed in {delay}.",
                    failedState.Exception, 
        ...
        private void TransitionToDeleted(ElectStateContext context, FailedState failedState)
        {
            ...
            {
                Logger.WarnException(
                    "Failed to process the job '{backgroundJobId}': an exception occured. Job was automatically deleted because the retry attempt count exceeded {attempts}.",
                    failedState.Exception,
                    context.BackgroundJob.Id,
                    Attempts);
            }
