...
from [{_storage.SchemaName}].{_storage.CustomTableNames["JobQueue"]} 
with (nolock)";
...
$@"select r.JobId from (
  ...
  from [{_storage.SchemaName}].{_storage.CustomTableNames["JobQueue"]} jq with (nolock, forceseek)
...
from (
    ...
    from [{_storage.SchemaName}].{_storage.CustomTableNames["JobQueue"]} with (nolock, forceseek)
    where Queue = @queue
) q";
